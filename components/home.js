...
import PhaseIndicator, { phaseConfig } from '../components/cycle/PhaseIndicator';
import { calculateCycleInfo } from '../components/cycle/CycleUtils';
import ActivityCard, { typeEmojis } from '../components/activities/ActivityCard';
              ...
              {todayActivities.slice(0, 3).map((sa) => (
                ...
                >
                  ...
                  <div className={`w-3 h-3 rounded-full ${
                    sa.is_completed 
                      ? 'bg-emerald-500' 
                      : `bg-gradient-to-r ${phaseConfig[currentPhase]?.color}`
                  }`} />
                </motion.div>
              ))}
            </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-slate-800">
              {phaseConfig[currentPhase]?.emoji} For Your {currentPhase?.replace(/^\w/, c => c.toUpperCase())} Phase
            </h2>
            <Link
